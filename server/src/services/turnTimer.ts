...
import { rollDice, calculateScore, autoSelectCategory, computeTotalScore, computeUpperTotal } from '../game/yatzyLogic';
import { ScoreCategory } from '../types';

// In-memory timer map: gameId -> NodeJS.Timeout
const timers = new Map<string, NodeJS.Timeout>();

const TURN_TIME_LIMIT = 60_000; // 60 seconds

/**
 * Start a 60-second timer for the current turn.
 * When it expires, auto-select the least valuable category.
 */
export function startTimer(gameId: string): void {
  clearTimer(gameId);
...
async function handleTimeout(gameId: string): Promise<void> {
  try {
    ...
    let diceValues = game.dice_values as number[];

    // If player hasn't rolled yet, roll for them
    if (game.current_roll === 0) {
      diceValues = rollDice([0, 0, 0, 0, 0], [false, false, false, false, false]);
    }
    ...
    const usedCategories = (scores || []).map((s) => s.category as ScoreCategory);

    // Auto-select the least valuable category
    const category = autoSelectCategory(diceValues, usedCategories);
    if (!category) return; // All categories filled
    ...
    if (!lobbyPlayers || lobbyPlayers.length !== 2) return;
